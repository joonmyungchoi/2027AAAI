...
1. Make sure you are within the `knowu_bench` repo directory and run `sudo mobile-world env run --image ghcr.io/yaosqz/knowu-bench:v1.1 --dev` will launch a dev container with the existing AVD image. This dev container will have your local `src` mounted to `/app/service/src`.
...
8. Compile a new image with the new AVD folder via `sudo docker buildx build -t ghcr.io/yaosqz/knowu-bench:v1.2 -f docker/Dockerfile .`.
